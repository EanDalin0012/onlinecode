...
//            String[] originalFilename = multipartFile.getOriginalFilename().split("\\.(?=[^\\.]+$)");
    ...
    public String Testing1 ( @RequestBody String data, HttpServletRequest request) throws Exception {
        try
        {
            ...
            new FileOutputStream(dir+"/image.jpg").write(imageByte);
        }
        catch(Exception e)
        {
            throw e;
    ...
    @GetMapping("/resource1/{resource_id}")
    public String  resourcesImage1(@PathVariable("resource_id") String resource_id) {
        String data = "";
        try {
            log.info("=======Start get resource ========");
            log.info("========== resource id :"+resource_id);
            MMap input = new MMap();
            input.setString("uuid",resource_id);
            data = resourceImageServiceImplement.getResourcesImageById(input);
            MMap resData = new MMap();
            resData.setString("file_source", data);
            log.info("=======file resource ========"+data);
            log.info("=======End get resource ========");
    ...
    public String writeTesting(@RequestBody MMap param) throws IOException {
        try {
            System.out.println("Enter base64 string to be converted to image");
            log.info("SystemUtil.resourcePath():"+SystemUtil.projectPath());
            ...
            writeByteToImageFile(base64Val, dir+"/image.png");
